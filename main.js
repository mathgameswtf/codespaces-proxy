// server.js
// Suppress util._extend deprecation warning from http-proxy-middleware
const originalEmit = process.emit;
process.emit = function (name, warning, ...args) {
  if (name === 'warning' && warning?.name === 'DeprecationWarning' && warning?.message?.includes('util._extend')) {
    return false;
  }
...
// Cache proxy instances by target base URL
const proxyCache = new Map();

function getProxy(targetBase, targetPath) {
  if (!proxyCache.has(targetBase)) {
    proxyCache.set(targetBase, createProxyMiddleware({
      target: targetBase,
      changeOrigin: true,
      ...
      on: {
        proxyReq: (proxyReq, req) => {
          proxyReq.path = req._targetPath;
        },
        proxyRes: (proxyRes, req, res) => {
          // ... same proxyRes handler as before
        }
      }
    }));
  }
  ...
  return (req, res, next) => {
    ...
    proxyCache.get(targetBase)(req, res, next);
  };
}
...
app.use('/proxy', (req, res, next) => {
  ...
  console.log(`[PROXY] ${req.method} ${raw.substring(0, 100)}`);
  if (/^https?:\/\//.test(raw)) {
    try {
